...
import type { Orden, OrdenBase } from '../src/tipos/ordenes.ts';
...
const UNO = 'casa-uno' as IdJugador;
const DOS = 'casa-dos' as IdJugador;
...
function recua(id: string, cambios: Partial<Recua> = {}): Recua {
  return {
    id: id as IdRecua,
    jugador: UNO,
    nombre: `Recua ${id}`,
...
let contador = 0;
function base(turno: number, coste: Partial<Recursos> = {}): OrdenBase {
  contador += 1;
  return {
...
function ordenRuta(turno: number, recuaId: string, paradas: string[], circular = false): Orden {
  return {
    ...base(turno),
    tipo: 'ruta',
    recua: recuaId as IdRecua,
    rebanyo: null,
    paradas: paradas.map((comarca) => ({
      comarca: c(comarca),
      cargar: {},
      descargar: {},
      vender: {},
...
function ordenFormar(turno: number, vecinos = 0): Orden {
  return {
...
function ordenCarga(turno: number, recuaId: string, cargar: Partial<Recursos>): Orden {
  return {
    ...base(turno),
    tipo: 'carga',
    recua: recuaId as IdRecua,
...
function turno(estado: EstadoPartida, ordenes: readonly Orden[] = [], tablas = reglas) {
  return resolverTurno(estado, ordenes, mundo, tablas);
}

function de(estado: EstadoPartida, id: string): Recua {
